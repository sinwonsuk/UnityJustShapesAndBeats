...
public class BoxFadePattern2 : MonoBehaviour
{
	...
	private void OnDisable()
	{
		...
		Debug.Log("Box Pattern 2 끝 및 오브젝트 클린 작업 완료");
	}
	...
	private IEnumerator ExecuteFadeBox(bool isHorizontal, Vector3 spawnPosition)
	{
		...
		yield return new WaitForSeconds(0.05f); // 대기 시간 줄임 (0.1f → 0.05f)
		Destroy(fadeBox);
	}
	...
	{
		float duration = 0.4f; // 애니메이션 지속 시간 (0.5f → 0.4f로 줄임)
		float elapsed = 0f;
		...
		if (box != null)
		{
			...
			Vector3 fadeEndScale = new Vector3(isHorizontal ? 18f : 0f, isHorizontal ? 0f : 9f, 1f);
